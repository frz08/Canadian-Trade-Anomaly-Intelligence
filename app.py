...
.card-name       { font-size: 11px; color: #444; margin-top: 4px; }
...
def load_data():
    try:
        anomalies    = pd.read_parquet("data/fact_anomalies")
        trade_series = pd.read_parquet("data/trade_series")
    except Exception:
        anomalies    = pd.read_csv("data/fact_anomalies.csv")
        trade_series = pd.read_csv("data/trade_series.csv")
...
chart_col, cards_col = st.columns([4, 1], gap="large")
...
with chart_col:
    if len(filtered) == 0:
        ...
        # Debug: show which chapter is being used for the background line
        if len(bg) == 0:
            st.markdown(
                f"<div style='font-size:11px;color:#555;margin-top:4px;'>"
                f"No trade series data found for <b style='color:#888'>{chart_chapter}</b> — "
                f"background line hidden. Select a specific chapter in the sidebar to see it.</div>",
                unsafe_allow_html=True
            )

with cards_col:
    st.markdown(f"<div style='font-size:11px;color:#555;margin-bottom:12px;'>"
                f"{len(filtered):,} anomalies · sorted by date</div>", unsafe_allow_html=True)
    ...
    if len(filtered) == 0:
        ...
        # Slider for the rest
        remaining = filtered.iloc[3:].reset_index(drop=True)
        if len(remaining) > 0:
            st.markdown("<div style='height:8px;'></div>", unsafe_allow_html=True)
            st.markdown("<div style='font-size:10px;color:#444;letter-spacing:0.06em;"
                        "text-transform:uppercase;margin-bottom:4px;'>More anomalies</div>",
                        unsafe_allow_html=True)
            idx = st.slider("", min_value=0, max_value=len(remaining)-1,
                            value=0, key="card_slider",
                            label_visibility="collapsed")
            render_card(remaining.iloc[idx])
